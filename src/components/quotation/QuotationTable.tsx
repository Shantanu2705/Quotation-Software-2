...
import { Search, ChevronDown, Plus, Pencil, Trash2, Download } from "lucide-react";
...
export interface Quotation {
  ...
  paymentStatus?: "In Progress" | "Received" | "Refunded" | "Cancelled";
  ...
  const handleDownloadQuotationPdf = async (q: Quotation) => {
    const data = {
      ...
      details: {
        ...
        "Travel Dates": q.travelStartDate ? `${q.travelStartDate} to ${q.travelEndDate}` : "TBD",
        "Duration": q.packageDuration,
        "Pax": `${q.adults || q.pax || 0} Adults${q.children ? `, ${q.children} Children` : ''}`,
        "Vehicle": q.vehicleType || "As per itinerary"
      },
      tableHeaders: ["Description", "Amount"],
      tableRows: [
        ["Tour Package Quote", q.subtotal ? `Rs. ${q.subtotal.toLocaleString("en-IN")}` : "0"]
      ],
      subtotal: q.subtotal || 0,
      tax: (q.grandTotal || 0) - (q.subtotal || 0),
      total: q.grandTotal || 0
    };
    await generateAndDownloadPdf(data, settings, `Quotation_${q.id}`);
  };

  const filters = [
    { defaultLabel: "Type", value: typeFilter, setter: setTypeFilter, options: ["Type", "Tour Package Enquiry", "Transport Enquiry"] },
    { defaultLabel: "Status", value: statusFilter, setter: setStatusFilter, options: ["Status", "Draft", "Sent", "Confirmed", "Cancelled"] },
  ];
                ...
                <TableRow key={q.id} className="border-slate-100 dark:border-border group">
                  ...
                  <TableCell className="px-4">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-36 rounded-xl">
                        {(["Draft", "Sent", "Confirmed", "Cancelled"] as QuotationStatus[]).map((status) => (
                  ...
                  <TableCell className="text-right whitespace-nowrap px-4">
                    <div className="flex items-center justify-end gap-2">
                      <Button variant="ghost" size="sm" className="h-7 px-2.5 text-[11px] text-blue-600 hover:text-blue-800 hover:bg-blue-50 font-semibold" onClick={() => handleDownloadQuotationPdf(q)}>
